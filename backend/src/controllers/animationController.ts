...
export class AnimationController {
   ...
   */
  async generateAnimation(req: Request<{}, {}, GenerateRequest>, res: Response<GenerateResponse | ApiError>): Promise<void> {
    try {
   ...
   */
  async getJobStatus(req: Request<{ id: string }>, res: Response<JobStatus | ApiError>): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const jobStatus: JobStatus = {
        ...
        videoUrl: job.status === 'done' && job.outputPath ? 
          `/outputs/${id}/${job.outputPath.split('/').pop()}` : undefined,
        error: job.error,
